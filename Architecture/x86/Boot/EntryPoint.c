...
void secondEntryPoint(unsigned magic, unsigned mbs)
ATTR_SECTION(".start");

ATTR_NORETURN void secondEntryPoint(unsigned magic, unsigned mbs) {
 if (magic != MULTIBOOT_MAGIC_CONSTANT  // If the magic value is not correct...
   || (void *) mbs == NULL  // ... or if it's pointer is null...
   || ((void **) mbs >= &imageStart && (void **) mbs <= &imageEnd)) {  // ... or if inside the image...
  miserableFail();
  BUILTIN_UNREACHABLE;
 } else {
