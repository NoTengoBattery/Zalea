...
static inline size_t unalignedLoop(
        void *buffer, unsigned char fill, size_t size, unsigned int alignment) {
...
void *__memset(void *buffer, int fill, size_t size) {
    ...
    if (shouldBeZero != 0x00) { return (void *) UINTPTR_MAX; }
    return buffer;
}
