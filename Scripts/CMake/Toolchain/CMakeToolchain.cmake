...
IF (TREE_SELF_PATH AND NOT TOOLCHAIN_DONE) # This will define if we have access to the scope variables and cache
  ...
  IF ("${KERNEL_COMPILER}" STREQUAL "Clang")
    ...
    SET(CMAKE_RANLIB "llvm-ranlib;ranlib")
  ...
  ELSEIF ("${KERNEL_COMPILER}" STREQUAL "Intel")
    SET(CMAKE_AR "xiar;ar")
    SET(CMAKE_RANLIB "xiar;ar")
    SET(CMAKE_ASM_COMPILER_AR "${CMAKE_AR}")
    SET(CMAKE_ASM_COMPILER_RANLIB "${CMAKE_RANLIB}")
    SET(CMAKE_C_COMPILER_AR "${CMAKE_AR}")
    SET(CMAKE_C_COMPILER_RANLIB "${CMAKE_RANLIB}")
    SET(CMAKE_CXX_COMPILER_AR "${CMAKE_AR}")
    SET(CMAKE_CXX_COMPILER_RANLIB "${CMAKE_RANLIB}")
    SET(CMAKE_ASM_ARCHIVE_FINISH "<CMAKE_RANLIB> s <TARGET>")
    SET(CMAKE_ASM_COMPILER "icc;icpc")
    SET(CMAKE_C_ARCHIVE_FINISH "<CMAKE_RANLIB> s <TARGET>")
    SET(CMAKE_C_COMPILER "icc;icpc")
    SET(CMAKE_CXX_ARCHIVE_FINISH "<CMAKE_RANLIB> s <TARGET>")
  ...
  IF ("${KERNEL_BINUTILS}" STREQUAL "LLVM")
    ...
    SET(CMAKE_READELF "llvm-readelf;readelf")
  ...
  FOREACH (ARCHITECTURE_NAME ${ARCHS})
    LIST(APPEND CMAKE_QEMU "qemu-system-${ARCHS}")
  ENDFOREACH ()
  ...
  IF ("${KERNEL_COMPILER}" STREQUAL "Clang"
      ...
      OR "${KERNEL_COMPILER}" STREQUAL "GNU")
    GET_FILENAME_COMPONENT(CMAKE_BINUTILS_BIN_PATH "${CMAKE_OBJCOPY}" DIRECTORY)
