...
IF (TREE_SELF_PATH) # This will define if we have access to the scope variables and cache
  ...
  IF ("${KERNEL_BINUTILS}" STREQUAL "GNU")
    SET(CMAKE_AS "${KERNEL_TARGET}-as")
    MESSAGE(STATUS "Trying to find tools for the target '${KERNEL_TARGET}'...")
    GUESS_TOOL_BY_NAME(AS BINUTILS)
    IF (NOT CMAKE_AS)
      SET(CMAKE_AS "${KERNEL_ALTERNATIVE_TARGET}-as")
      MESSAGE(STATUS "Trying to find tools for the target '${KERNEL_ALTERNATIVE_TARGET}'...")
      GUESS_TOOL_BY_NAME(AS BINUTILS)
      IF (NOT CMAKE_AS)
        SET(CMAKE_AS "${KERNEL_SECOND_TARGET}-as")
        MESSAGE(STATUS "Trying to find tools for the target '${KERNEL_SECOND_TARGET}'...")
        GUESS_TOOL_BY_NAME(AS BINUTILS)
        IF (CMAKE_AS)
          SET(_KERNEL_TARGET "${KERNEL_SECOND_TARGET}")
        ENDIF ()
      ELSE ()
        SET(_KERNEL_TARGET "${KERNEL_ALTERNATIVE_TARGET}")
      ENDIF ()
    ELSE ()
      SET(_KERNEL_TARGET "${KERNEL_TARGET}")
    ENDIF ()
